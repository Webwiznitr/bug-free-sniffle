function Statistics() {
  return (
    <div className="flex h-full w-full flex-col gap-10 pl-10">
      ...
      <div className="grid w-fit grid-cols-2 gap-x-10 gap-y-8">
        {[1, 2, 3, 4].map((val, index) => {
          return (
            <div
              key={index}
              className={`size-[9.5rem] -rotate-4 shadow-[4px_4px_0_black] [@media(max-width:563px)]:size-[6.75rem] ${index == 0 || index == 2 ? 'bg-primary' : 'bg-secondary'}`}
            />
          );
        })}
      </div>
